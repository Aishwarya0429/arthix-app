...
def show():
    ...
    # Top bar
    st.markdown(f"""
    <div class="arthix-topbar">
        <div>
            <div class="arthix-topbar-title">Dashboard</div>
            <div class="arthix-topbar-sub">{biz} · Live business pulse</div>
        </div>
    </div>
    ...
    cards = [
        (c1, "&#8358;", "#dbeafe", "#1d4ed8", "Total Revenue",
         kpi["month_income"],  kpi.get("income_growth", 0)),
        (c2, "&#9653;", "#fee2e2", "#b91c1c", "Total Expenses",
         kpi["month_expense"],
         ((kpi["month_expense"]-kpi["last_expense"])/kpi["last_expense"]*100
          if kpi["last_expense"] else 0)),
        (c3, "&#9651;", "#d1fae5", "#065f46", "Net Profit",
         kpi["month_profit"],  kpi.get("income_growth", 0)),
        (c4, "&#37;",   "#ede9fe", "#5b21b6", "Profit Margin",
         kpi["margin"], 0),
    ]
    for col, icon, bg, color, label, value, delta in cards:
        with col:
            ...
            <div class="metric-card">
                <div class="metric-top">
                    ...
                    <div class="metric-badge" style="background:{bg}; color:{color}">
                        {d_html if delta != 0 else "Stable"}
                    </div>
                </div>
    ...
    with col_left:
        st.markdown('<div class="card-title">&#9641; Revenue vs Expenses (Last 30 Days)</div>',
                    unsafe_allow_html=True)
        daily = get_daily_summary(bid, days=30)
        if not daily.empty:
            fig = ch.area_chart(daily, "txn_date",
                                ["income", "expense", "profit"], title="")
            ch.render_chart(st, fig)
        else:
            st.info("Not enough data.")

    with col_right:
        st.markdown('<div class="card-title">&#9685; Expense Breakdown</div>',
                    unsafe_allow_html=True)
        _, exp_cat = get_category_summary(bid)
        if not exp_cat.empty:
            fig = ch.donut_chart(exp_cat["category"].tolist(),
                                  exp_cat["amount"].tolist(), title="")
            ch.render_chart(st, fig)
    ...
    with col_l2:
        st.markdown('<div class="card-title">&#9670; Top Income Sources</div>',
                    unsafe_allow_html=True)
        inc_cat, _ = get_category_summary(bid)
        if not inc_cat.empty:
            top = inc_cat.sort_values("amount", ascending=False).head(5)
            fig = ch.bar_chart(top, "category", "amount",
                               color="#2563eb", title="", orientation="h")
            ch.render_chart(st, fig)

    with col_r2:
        st.markdown('<div class="card-title">&#9650; 30-Day Profit Forecast</div>',
                    unsafe_allow_html=True)
        ...
        if not daily_all.empty and len(daily_all) >= 7:
            ...
            fig   = ch.forecast_chart(
                daily_all.tail(30), fcast, "txn_date", "income", title="")
            ch.render_chart(st, fig)
            if not fcast.empty:
                ...
                <div class="forecast-tag">
                    ...
                    <strong style="color:#2563eb">{fmt(next_m_profit)}</strong>
                </div>
                """, unsafe_allow_html=True)
    ...
    with col_al:
        st.markdown('<div class="card-title">&#9888; Inventory Alerts</div>',
                    unsafe_allow_html=True)
        ...
        if out:
            for p in out[:3]:
                st.markdown(
                    f'<div class="alert alert-danger">&#9632; <b>{p["name"]}</b> — Out of stock</div>',
                    unsafe_allow_html=True)
        if low:
            for p in low[:3]:
                st.markdown(
                    f'<div class="alert alert-warning">&#9650; <b>{p["name"]}</b> — Only {p["stock"]} left</div>',
                    unsafe_allow_html=True)
        if not low and not out:
            st.markdown(
                '<div class="alert alert-success">&#10003; All products adequately stocked</div>',
                unsafe_allow_html=True)

    with col_inv:
        st.markdown('<div class="card-title">&#9723; Inventory Overview</div>',
                    unsafe_allow_html=True)
        if products:
            df_prod = pd.DataFrame(products)[
                ["name", "stock", "sale_price", "low_stock_threshold"]]
            df_prod["Status"] = df_prod.apply(
                lambda r: "Out of Stock" if r["stock"] == 0
                else ("Low Stock" if r["stock"] <= r["low_stock_threshold"]
                      else "In Stock"), axis=1)
            df_prod["Stock Value"] = (
                df_prod["stock"] * df_prod["sale_price"]).apply(fmt)
            df_prod = df_prod.rename(columns={
                "name": "Product", "stock": "Stock", "sale_price": "Price"})
            st.dataframe(
                df_prod[["Product", "Stock", "Price", "Status", "Stock Value"]],
                use_container_width=True, hide_index=True)

    # ── Recent Transactions ────────────────────────────────────
    st.markdown("---")
    st.markdown('<div class="card-title">&#9644; Recent Transactions</div>',
                unsafe_allow_html=True)
    recent = txns[:10]
    if recent:
        df_r = pd.DataFrame(recent)[
            ["txn_date", "type", "category", "amount", "description"]]
